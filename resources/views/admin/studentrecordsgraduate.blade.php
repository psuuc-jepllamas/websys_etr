...
<head>
    ...
    <title>Admin Dashboard - PSU Graduate Student Records</title>
    ...
    <div class="d-flex flex-column min-vh-100">
        <header class="header py-2">
            <div class="container d-flex align-items-center">
                ...
                <h2 class="mb-0">Registrar's Office - Graduate Records</h2>
            </div>
        </header>
        ...
        <div class="d-flex flex-grow-1">
            ...
            <div class="main-content">
                <h1 class="mb-4">Graduate Program</h1>
                @if (session('error'))
                ...
                @endif
                <form class="filter-form mb-4" method="GET" action="{{ route('admin.studentrecordsgraduate') }}">
                    <div class="d-flex align-items-center">
                        <label for="grad_course" class="me-2">Program:</label>
                        <select name="grad_course" id="grad_course" onchange="this.form.submit()">
                            <option value="" {{ $gradCourse == '' ? 'selected' : '' }}>All Programs</option>
                            @foreach ($gradCourses as $course)
                                <option value="{{ $course }}" {{ $gradCourse == $course ? 'selected' : '' }}>{{ $course }}</option>
                            @endforeach
                        </select>
                ...
                <div class="table-container">
                    <table id="graduateTable" class="table table-bordered">
                        <thead>
                            <tr>
                                <th>Control No.</th>
                                <th>ID</th>
                        ...
                        <tbody>
                            @forelse ($gradstudents as $gradstudent)
                                <tr>
                                    <td>TC-{{ $gradstudent->course }}-{{ $gradstudent->id }}-G-{{ \Carbon\Carbon::parse($gradstudent->ordate)->year }}</td>
                                    <td>{{ $gradstudent->id }}</td>
                                    <td>{{ $gradstudent->fullname }}</td>
                                    <td>{{ $gradstudent->address }}</td>
                                    <td>{{ $gradstudent->course }}</td>
                                    <td>{{ $gradstudent->entrance }}</td>
                                    <td>{{ $gradstudent->orno }}</td>
                                    <td>{{ $gradstudent->ordate }}</td>
                                    <td>{{ $gradstudent->created_at }}</td>
                                    <td>Graduate</td>
                                    <td>
                                        <a href="{{ route('admin.printCredential', ['id' => $gradstudent->id, 'type' => 'graduates']) }}" class="btn btn-primary"><i class="bi bi-printer"></i></a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="10" class="text-center">No graduate records found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
